...
Process {
    ...
    /**
     * The helper's exit status from the last run, or -1 before one finishes.
     *
     * Process only hands this over as an argument to `exited`; it has no
     * property of its own to read afterwards, so it is kept here for
     * `finishedWith` handlers.
     */
