...
Item {
    ...
    function activate() {
        ...
        //
        // The URL is built from a constant origin and an id sanitize.js has
        // already refused to treat as anything but digits.
        var target = Sanitize.dashboardUrl(row.id);
        if (target === null) {
            return;
        }
        // Absolute path, so a planted `xdg-open` earlier on PATH is not the
        // program that runs. No `--`: xdg-open rejects it as an unknown option
        // and opens nothing. It is not needed either — dashboardUrl() only
        // ever returns a string starting "https://", which cannot parse as one.
    ...
    PanelWindow {
        ...
        color: "transparent"
        WlrLayershell.namespace: "duckcove-uptimerobot"
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        Text {
                            id: staleText
                            anchors.centerIn: parent
                            text: "Can't reach UptimeRobot — showing the last known state"
                    ...
                    SetupForm {
                        ...
                        shell: root.shell
                        moduleName: "duckcove.uptimerobot"
                        foreground: root.foreground
                        fontFamily: root.fontFamily
